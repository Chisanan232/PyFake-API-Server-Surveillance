...
class TestActionInput(_BaseModelTestSuite):
    ...
    def _verify_model_props(self, model: ActionInput, original_data: Mapping) -> None:
        ...
        assert model.subcmd_pull_args.include_template_config == bool(original_data[EnvironmentVariableKey.INCLUDE_TEMPLATE_CONFIG.value])
        ...
        assert model.subcmd_pull_args.divide_http_request == bool(original_data[EnvironmentVariableKey.DIVIDE_HTTP_REQUEST.value])
        assert model.subcmd_pull_args.divide_http_response == bool(original_data[EnvironmentVariableKey.DIVIDE_HTTP_RESPONSE.value])
