...
class TestGitOperation:
    ...
    def test_fake_api_server_monitor_git_branch(
        self, git_operation: GitOperation, ci_test_mode: str, expect_git_branch: str
    ):
        ...
        if ci_test_mode:
            expect_git_branch = expect_git_branch.replace("<event_name>", dummy_ci_env["GITHUB_EVENT_NAME"]).replace(
                "<job_id>", dummy_ci_env["GITHUB_JOB"]
            )
